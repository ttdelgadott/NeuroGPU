...
clear variables
clc
fidTopo = fopen('../Neuron/Figure1/Output/96RCT.csv');
[Neuron] = ReadTopologyMultipleStim(fidTopo);
fidConstants = fopen('../Neuron/Figure1/Output/96RCC.csv');
...
for i = 0:Stim.NStimuli-1
    fidFmatrix = ['../Neuron/printCell/output/96RCF' num2str(i) '.csv'];
    ...
    Table = readFmatrixOnly(fidFmatrix,sum(Neuron.NSegsMat));
    Table=Table(:,2:4);
end
...
%        Vmid = fliplr(SolveByUsP(Aux,Ds,rhss));
%         for seg=1:Nx
