function [] = MainPassive(topoFile,constFile,matrixFile)
close all
%clear variables
clc
fidTopo = fopen(topoFile);
[Neuron] = ReadTopologyMultipleStimPas(fidTopo);
...
%NRHS=squeeze(Mats(:,5,:));
 index=1;
    ...
    for comp=1:length(Neuron.NSegsMat)
        ...
        ParamsM(index:index+currNSegs-1,2) = ones(currNSegs,1)*Neuron.Es(comp,1);
...
% Aux = debugAux(Aux);
    ...
    %         isHH = ~isempty(find(Neuron.HasHH==seg));
            ...
            if(isHH)
                [sumCurrents sumConductivity]=BreakpointModelPas(Vs(seg),ParamsM(seg,1),ParamsM(seg,2));
                [sumCurrentsDv sumConductivityDv]=BreakpointModelPas(Vs(seg)+0.001, ParamsM(seg,1),ParamsM(seg,2));
                gHH = sumCurrentsDv;
                rhsHH=sumCurrents;
                gHH = (gHH-rhsHH)/0.001;
            else
