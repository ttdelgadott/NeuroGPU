...
[Neuron Bla Stim] = ReadKForkTopology2V2(FN_Topo);
...
if (KineticFlg)
    ...
    fprintf(fid,'#define _RHS1(arg) rhs[arg]\n#define _MATELM1(i, j) matq[i][j]\n\n\n');
...
% if (KineticFlg)
%     fprintf(fid,['float matq[',num2str(length(AllState)),'][',num2str(length(AllState)),'];\n']);
% end
...
for currParam = 1:nParams
    currParametrStr = sprintf('#define  SET_PARAMS%03d(VARILP) MYFTYPE p%d_ ## VARILP =ParamsM[NeuronID*perThreadParamMSize + %d*InMat.NComps+cSegToComp[PIdx_ ## VARILP] ];\n',currParam,currParam-1,currParam-1);
    fprintf(fid,currParametrStr);
end

% fprintf(fid,'#define SET_PARAMS(VARILP) MYFTYPE %s;%s\n\n',declareStr(1:end-1),parmeterSetStr);
callToInitStrCU = regexprep(callToInitStrCU,'p([0-9]*)_ ## VARILP','param_macro($1,PIdx_ ## VARILP)');
