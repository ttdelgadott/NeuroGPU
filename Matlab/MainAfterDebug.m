...
fidTopo = fopen('../Neuron/printCell/Output/SomaTopo.csv');
[Neuron Sim Stim] = ReadKForkTopology(fidTopo);
fidFmatrix = '../Neuron/printCell/output/fmatrixSoma.csv';
...
for i=1:(Nt-1)
    ...
    for seg=2:Nx%ATTENTION the indices here have offset of +1 regarding to the ones in nrn_rhs at treeset.c
        ...
        current = handleStim(Stim,seg,t);
        rhss(1,seg) = current;
        dv = Vs(max(parentIndex,1))-Vs(seg);
        rhss(1,seg) =rhss(1,seg) -OurMat(seg-1,seg)*dv;
        if(seg>1)
            rhss(1,seg-1) =rhss(1,seg-1) +OurMat(seg,seg-1)*dv;
        end
