...
for i=1:(Nt-1)
    ...
    RHSMatElement = Vs*OrigMat;
    RHSMEHot(i) = RHSMatElement(InLoc);
    di = sumCurrentsDv-sumCurrents;
    didv = di./0.001;
    didvHot(i)=didv(InLoc);
    sumConductivity(cmVec==0)=0;
    
    sumCurrents(cmVec==0)=0;
    OurMat(eye(Nx)>0)=OrigDiag'+didv+cmVec./(1000*Sim.dt);%Update matrix
    ...
    NewRHS=-sumCurrents +RHSMatElement;
...
figure(22);
plot(Sim.dt:Sim.dt:(Sim.Tfinal-Sim.dt),AllVs(10,:));
hold on;
