...
while ischar(tline)
    ...
    tline=fgetl(fid);
%(amit - i changed it to make the Topology file more generic, this line prints hh variables)
...
%  Neuron.Es = ModelParams(:,end-2:end);    Amit - no model params.
%  Neuron.GBars = ModelParams(:,1:2);       Amit - no model params.
CNMat = cumsum(Neuron.NSegsMat);
if(numel(CNMat)>1)
    Neuron.SegStart = [1 CNMat(1:end-1)'+1];
    %Neuron.SegStart = [1 CNMat(1:end-1)'];
...
fid=fopen(FNM,'wb');
% fwrite(fid,Neuron.Es,'float32');      Amit - no model params.
% fwrite(fid,Neuron.GBars,'float32');   Amit - no model params.
fclose(fid);
