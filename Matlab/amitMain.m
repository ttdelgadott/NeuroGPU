...
FN_TopoList =[BaseP 'Neuron/printCell/Amit/Output/64TL.csv'];
...
hasSynapses=false;
PreSynapseList=[17];
PreSynnapticS(1).Thresh=0;
PreSynnapticS(1).Target=[59];
PostSynapticList=[59];
PostSynapticS(1).Delay=0.5;
PostSynapticS(1).Weight=0.5;
PostSynapticS(1).Tau=0.1;
PostSynapticS(1).g=0;
PostSynapticS(1).e=0;
PostSynapticS(1).Update=@(S) S.g*exp(-1/S.Tau);
PostSynapticS(1).Current=@(S,v) S.g*(v-S.e);
%%
...
for i=1:(Nt)
    ...
    rhss = zeros(1,Nx);
    D = zeros(1,Nx);
    %calculate the reverse potential for all ions - at the moment only for
    %ca
    if (IS_ANY_MODEL_WRITING_CAI)
        constsAndVars = nernstCa(constsAndVars);
    end;
...
%         constsPerModelAllSegs, constsAllModelsAllSegs, ModelsFuncsNames, NUM_OF_MODELS, numOfSegs);
    [rhss dgs constsAndVars] = nrnRhs (rhss, dgs, t, Vs, hasModel,...
...
%     GGG Synapse: Check voltage here
end
toc;
%figure(11);
