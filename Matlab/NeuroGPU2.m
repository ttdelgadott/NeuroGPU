function [] = NeuroGPU2(topoFile,constFile,matrixFile)
close all
%clear variables
clc
fidTopo = fopen(topoFile);
[Neuron] = ReadTopologyMultipleStim(fidTopo);
fidConstants = fopen(constFile);
...
for i = 0:Stim.NStimuli-1
    fidFmatrix = [matrixFile num2str(i) '.csv'];
...
%[OurMat Table]= createMatrixAsNEURONNew(Neuron);
 %save('realreacell','Neuron', 'Sim', 'Stim');
...
if(NSTATES>0)
    ...
    fidParamsM=fopen(['../Data/ParamsM' num2str(Nx) '.mat'],'wb');
...
%                 [sumCurrentsDv sumConductivityDv]=BreakpointModel(Vs(seg)+0.001, StatesM(seg,1), StatesM(seg,2), StatesM(seg,3), ParamsM(seg,1), ParamsM(seg,2), ParamsM(seg,3), ParamsM(seg,4), ParamsM(seg,5), ParamsM(seg,6));
                [sumCurrents sumConductivity]=BreakpointModel(Vs(seg), MidStatesM(seg,1), StatesM(seg,2), StatesM(seg,3), ParamsM(seg,1), ParamsM(seg,2), ParamsM(seg,3), ParamsM(seg,4), ParamsM(seg,5), ParamsM(seg,6));
                [sumCurrentsDv sumConductivityDv]=BreakpointModel(Vs(seg)+0.001, MidStatesM(seg,1), StatesM(seg,2), StatesM(seg,3), ParamsM(seg,1), ParamsM(seg,2), ParamsM(seg,3), ParamsM(seg,4), ParamsM(seg,5), ParamsM(seg,6));
                gHH = sumCurrentsDv;
                rhsHH=sumCurrents;
                gHH = (gHH-rhsHH)/0.001;
            else
...
%                 Now with CN
               D = sumConductivity+cmVec(seg)/(1000*Sim.dt*0.5);
    ...
    %         disp('Diff D');
    %     end
       %[OurDiag Vmid] = solveByNeuron(Nx,Aux,fliplr(Ds),fliplr(rhss),Neuron);
%        Vmid = fliplr(SolveByUs(Aux,Ds,rhss));
       VmidP = fliplr(SolveByUsP(Aux,Ds,rhss));
%         for seg=1:Nx
...
%         Vs=Vs+VmidToUse; %like 2*Vmid - Vs ???;
        % Now with CN
        Vs=Vs+2*VmidToUse; %like 2*Vmid - Vs ???; First time - advancing half

        for seg=1:Nx
            isHH=Aux.boolHasHH(seg);
            if(isHH)            
%                 [StatesM(seg,1) StatesM(seg,2) StatesM(seg,3)]=DerivModel(Sim.dt, Vs(seg),  StatesM(seg,1), StatesM(seg,2), StatesM(seg,3));
                [MidStates{1:NSTATES}]=DerivModel(Sim.dt*0.5, Vs(seg),  StatesM(seg,1), StatesM(seg,2), StatesM(seg,3)); % half-step backward Euler
                MidStatesM(seg,1:NSTATES)=[MidStates{:}];
%                 [MidStates2{1:NSTATES}]=DerivModel(Sim.dt, Vs(seg),  StatesM(seg,1), StatesM(seg,2), StatesM(seg,3)); % half-step backward Euler
                StatesM(seg,1:NSTATES)=2*[MidStates{:}]-StatesM(seg,1:NSTATES); % second half-step forward Euler
            end
        end
    ...
    Vs=Vs+VmidToUse; %like 2*Vmid - Vs ???; Second time - advancing second half
