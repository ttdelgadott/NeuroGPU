	...
	Prop *_prop;
{
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 2);
 	...
 	ivoc_help("help ?1 Khh /cygdrive/c/Users/Roy/Documents/NeuroGPU2_2/Neuron/printCell/Amit/Khh.mod\n");
 ...
 v=_v;
{
 ...
 for (; t < _break; t += dt) {
 error =  states();
 if(error){fprintf(stderr,"at line 44 in file Khh.mod:\n        SOLVE states\n"); nrn_complain(_p); abort_run(error);}
...
static _initlists() {
 int _i; static int _first = 1;
  if (!_first) return;
   _t_ninf = makevector(701*sizeof(double));
   _t_nexp = makevector(701*sizeof(double));
_first = 0;
}
