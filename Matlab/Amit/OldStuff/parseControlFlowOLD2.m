function Lines = parseControlFlowNew(Lines, startLin, startLet, endLin, endLet)
...
while ((lineInd<endLin) || (lineInd==endLin && letterInd<=endLet))
    ...
    if (~isempty(ifStatementInd))
        ...
        letterInd = letterInd + numel(IF_STR);  %QWERTY - letterInd shift is ok?
        ...
        letterInd = endOfBlockLetter + numel(END_MATLAB); %QWERTY - might be problematic if the recursive parse inserts another end in the same line
    else
        lineInd = lineInd + 1;
        letterInd = 1;
    end
