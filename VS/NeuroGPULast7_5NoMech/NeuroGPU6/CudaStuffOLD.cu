...
#endif
#ifdef BKSUB2
__device__ void BkSub(HMat InMat, MYFTYPE* uHP, MYFTYPE* bHP, MYFTYPE* bPX, MYDTYPE Depth)
{
	MYDTYPE PIdx=threadIdx.x;
	MYDTYPE PIdx_2=threadIdx.x+WARPSIZE;
#ifdef ILP3
	...
	MYDTYPE PIdx_32=threadIdx.x+(WARPSIZE*31);
#endif
	bPX[PIdx]=0; // might be useless?
	bPX[PIdx_2]=0; // might be useless?
#ifdef ILP3
	bPX[PIdx_3]=0; // might be useless?
#endif
#ifdef ILP4
	bPX[PIdx_4]=0; // might be useless?
#endif
#ifdef ILP5
	bPX[PIdx_5]=0; // might be useless?
#endif
#ifdef ILP6
	bPX[PIdx_6]=0; // might be useless?
#endif
#ifdef ILP7
	bPX[PIdx_7]=0; // might be useless?
#endif
#ifdef ILP8
	bPX[PIdx_8]=0; // might be useless?
#endif
#ifdef ILP9
	bPX[PIdx_9]=0; // might be useless?
#endif
#ifdef ILP10
	bPX[PIdx_10]=0; // might be useless?
#endif
#ifdef ILP11
	bPX[PIdx_11]=0; // might be useless?
#endif
#ifdef ILP12
	bPX[PIdx_12]=0; // might be useless?
#endif
#ifdef ILP13
	bPX[PIdx_13]=0; // might be useless?
#endif
#ifdef ILP14
	bPX[PIdx_14]=0; // might be useless?
#endif
#ifdef ILP15
	bPX[PIdx_15]=0; // might be useless?
#endif
#ifdef ILP16
	bPX[PIdx_16]=0; // might be useless?
#endif
#ifdef ILP17
	bPX[PIdx_17]=0; // might be useless?
#endif
#ifdef ILP18
	bPX[PIdx_18]=0; // might be useless?
#endif
#ifdef ILP19
	bPX[PIdx_19]=0; // might be useless?
#endif
#ifdef ILP20
	bPX[PIdx_20]=0; // might be useless?
#endif
#ifdef ILP21
	bPX[PIdx_21]=0; // might be useless?
#endif
#ifdef ILP22
	bPX[PIdx_22]=0; // might be useless?
#endif
#ifdef ILP23
	bPX[PIdx_23]=0; // might be useless?
#endif
#ifdef ILP24
	bPX[PIdx_24]=0; // might be useless?
#endif
#ifdef ILP25
	bPX[PIdx_25]=0; // might be useless?
#endif
#ifdef ILP26
	bPX[PIdx_26]=0; // might be useless?
#endif
#ifdef ILP27
	bPX[PIdx_27]=0; // might be useless?
#endif
#ifdef ILP28
	bPX[PIdx_28]=0; // might be useless?
#endif
#ifdef ILP29
	bPX[PIdx_29]=0; // might be useless?
#endif
#ifdef ILP30
	bPX[PIdx_30]=0; // might be useless?
#endif
#ifdef ILP31
	bPX[PIdx_31]=0; // might be useless?
#endif
#ifdef ILP32
	bPX[PIdx_32]=0; // might be useless?
#endif
 ...
 for(CurLevel=Depth;CurLevel>=0;CurLevel--) {
  ...
  for(LRelIndex=InMat.LRelStarts[CurLevel];LRelIndex<=InMat.LRelEnds[CurLevel];LRelIndex++){ 
   ...
   bPX[i]=(bHP[i]-bPX[k]*InMat.f[i])/uHP[i];
   }
  }
 }
 } 
...
__global__ void stEfork2TimeLoopGPUKernel(Stim stim, MYFTYPE* ParamsM, Sim sim, HMat InMat, MYFTYPE *V,MYFTYPE* VHotGlobal)
{
	...
	if(PIdx<InMat.nLRel) { // up to 32 !!!
		SMemMat.LRelStarts[PIdx]=InMat.LRelStarts[PIdx];
		SMemMat.LRelEnds[PIdx]=InMat.LRelEnds[PIdx];
	}
	if(PIdx<InMat.nFLRel) { // up to 32 !!!
		SMemMat.FLRelStarts[PIdx]=InMat.FLRelStarts[PIdx];
		SMemMat.FLRelEnds[PIdx]=InMat.FLRelEnds[PIdx];
	}
	#ifdef BKSUB2
		SMemMat.KsB =  (MYDTYPE*) &smem[offset];
		offset+=(SMemMat.N+1)*sizeof(MYDTYPE);
		SMemMat.KsB[PIdx]=InMat.KsB[PIdx];
		SMemMat.KsB[InMat.N]=InMat.KsB[InMat.N];
		SMemMat.KsB[PIdx_2]=InMat.KsB[PIdx_2];
	#ifdef ILP3
		SMemMat.KsB[PIdx_3]=InMat.KsB[PIdx_3];
	#endif
	#ifdef ILP4
		SMemMat.KsB[PIdx_4]=InMat.KsB[PIdx_4];
	#endif
	#ifdef ILP5
		SMemMat.KsB[PIdx_5]=InMat.KsB[PIdx_5];
	#endif
	#ifdef ILP6
		SMemMat.KsB[PIdx_6]=InMat.KsB[PIdx_6];
	#endif
	#ifdef ILP7
		SMemMat.KsB[PIdx_7]=InMat.KsB[PIdx_7];
	#endif
	#ifdef ILP8
		SMemMat.KsB[PIdx_8]=InMat.KsB[PIdx_8];
	#endif
	#ifdef ILP9
		SMemMat.KsB[PIdx_9]=InMat.KsB[PIdx_9];
	#endif
	#ifdef ILP10
		SMemMat.KsB[PIdx_10]=InMat.KsB[PIdx_10];
	#endif
	#ifdef ILP11
		SMemMat.KsB[PIdx_11]=InMat.KsB[PIdx_11];
	#endif
	#ifdef ILP12
		SMemMat.KsB[PIdx_12]=InMat.KsB[PIdx_12];
	#endif
	#ifdef ILP13
		SMemMat.KsB[PIdx_13]=InMat.KsB[PIdx_13];
	#endif
	#ifdef ILP14
		SMemMat.KsB[PIdx_14]=InMat.KsB[PIdx_14];
	#endif
	#ifdef ILP15
		SMemMat.KsB[PIdx_15]=InMat.KsB[PIdx_15];
	#endif
	#ifdef ILP16
		SMemMat.KsB[PIdx_16]=InMat.KsB[PIdx_16];
	#endif
	#ifdef ILP17
		SMemMat.KsB[PIdx_17]=InMat.KsB[PIdx_17];
	#endif
	#ifdef ILP18
		SMemMat.KsB[PIdx_18]=InMat.KsB[PIdx_18];
	#endif
	#ifdef ILP19
		SMemMat.KsB[PIdx_19]=InMat.KsB[PIdx_19];
	#endif
	#ifdef ILP20
		SMemMat.KsB[PIdx_20]=InMat.KsB[PIdx_20];
	#endif
	#ifdef ILP21
		SMemMat.KsB[PIdx_21]=InMat.KsB[PIdx_21];
	#endif
	#ifdef ILP22
		SMemMat.KsB[PIdx_22]=InMat.KsB[PIdx_22];
	#endif
	#ifdef ILP23
		SMemMat.KsB[PIdx_23]=InMat.KsB[PIdx_23];
	#endif
	#ifdef ILP24
		SMemMat.KsB[PIdx_24]=InMat.KsB[PIdx_24];
	#endif
	#ifdef ILP25
		SMemMat.KsB[PIdx_25]=InMat.KsB[PIdx_25];
	#endif
	#ifdef ILP26
		SMemMat.KsB[PIdx_26]=InMat.KsB[PIdx_26];
	#endif
	#ifdef ILP27
		SMemMat.KsB[PIdx_27]=InMat.KsB[PIdx_27];
	#endif
	#ifdef ILP28
		SMemMat.KsB[PIdx_28]=InMat.KsB[PIdx_28];
	#endif
	#ifdef ILP29
		SMemMat.KsB[PIdx_29]=InMat.KsB[PIdx_29];
	#endif
	#ifdef ILP30
		SMemMat.KsB[PIdx_30]=InMat.KsB[PIdx_30];
	#endif
	#ifdef ILP31
		SMemMat.KsB[PIdx_31]=InMat.KsB[PIdx_31];
	#endif
	#ifdef ILP32
		SMemMat.KsB[PIdx_32]=InMat.KsB[PIdx_32];
	#endif
#endif
	...
	MYDTYPE PerStimulus=((InMat.N+2)*2+2*InMat.N+WARPSIZE)*sizeof(MYFTYPE);
...
#endif
#ifdef BKSUB2
	PX=(MYFTYPE*) &smem[offset];
	offset+=SMemMat.N*sizeof(MYDTYPE);
#endif
	PF=uHP;
	MYFTYPE *SMemVHot;
...
#endif
#ifdef BKSUB2
		BkSub(SMemMat,uHP,bHP,PX, SMemMat.Depth);
#endif
...
#ifdef BKSUB2
	offset+=(InMat.N)*sizeof(MYFTYPE);//PX should be initialized with zerox	
#endif
