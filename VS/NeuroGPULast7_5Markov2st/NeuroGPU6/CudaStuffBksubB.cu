...
__device__ void BkSub(HMat InMat, MYFTYPE* uHP, MYFTYPE* bHP,  MYFTYPE* PX, MYFTYPE* PF,MYDTYPE Depth)
{
	MYDTYPE PIdx=threadIdx.x;
	...
	for(CurLevel=Depth;CurLevel>=0;CurLevel--) {
		...
		for(i=InMat.SegEndI[JumctionI]-2;i>=InMat.SegStartI[JumctionI]-2;i--) {
			// k=KsB(i+1);
			MYDTYPE k=InMat.KsB[i+1];
			// bPX(i)=(bH(i)-bPX(k)*f(i))/uH(i);
			bHP[i]=(uHP[i]-bHP[k]*InMat.f[i])/uHP[i];
		}
	}
}

__device__ void BkSubA(HMat InMat, MYFTYPE* PX, MYFTYPE* PF,MYFTYPE* uHP, MYFTYPE* bHP, MYDTYPE LognDepth)
{
	MYDTYPE PIdx=threadIdx.x;
	MYDTYPE PIdx_2=threadIdx.x+WARPSIZE;
#ifdef ILP3
...
__global__ void stEfork2TimeLoopGPUKernel(Stim stim, MYFTYPE* ParamsM, Sim sim, HMat InMat, MYFTYPE *V,MYFTYPE* VHotGlobal)
{
	...
	//RRR you need to get KsB intoshared mem
