...
float gbar_kv,float tha_kv,float qa_kv,float Ra_kv,float Rb
f
l
o
a
...
void InitModel_kv(float v,float &n,float gbar_kv,float tha_kv,float qa_kv,float Ra_kv,float Rb_kv){
	...
	trates_kv(v,gbar_kv,tha_kv,qa_kv,Ra_kv,Rb_kv,gbar_kv,tha_kv,qa_kv,Ra_kv,Rb_kv);
	n = ninf;
};
...
void BreakpointModel_kv(MYSECONDFTYPE &sumCurrents, MYFTYPE &sumConductivity, float v,float &n,float gbar_kv,float tha_kv,float qa_kv,float Ra_kv,float Rb_kv) {gk=tadj_kv*gbar_kv*n;
ik=(1e-4)*gk*(v-ek);
sumCurrents+= ik;
sumConductivity+= gk;
};
