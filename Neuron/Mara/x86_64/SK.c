	...
	Prop *_prop;
{
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_size(_mechtype, 5);
 	...
 	ivoc_help("help ?1 sk /Users/alon/Neuron/mara/stage2_bk_inact_line/Demo/x86_64/SK.mod\n");
...
static int  rate ( _p, _ppvar, _thread, _nt, _lcai ) double* _p; Datum* _ppvar; Datum* _thread; _NrnThread* _nt; 
	double _lcai ;
 {
   double _lq10 ;
 _lq10 = pow( Cq10 , ( ( celsius - 22.0 ) / 10.0 ) ) ;
   a = a0 * pow( _lcai , 4.0 ) ;
   tau = _lq10 / ( a + b0 ) ;
   inf = a / ( a + b0 ) ;
    return 0; }
